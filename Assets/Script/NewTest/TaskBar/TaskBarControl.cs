...
public class TaskBarControl : MonoBehaviour
{
    ...
    public void ToggleDictation()
    {
        ...
        {
            dictation.Deactivate(); // Let session stop naturally
            DebugLogger.Log("Dictation stopping...");
        }
    ...
    private void UnbindListeners()
    {
        ...
        dictation.DictationEvents.OnDictationSessionStopped.RemoveListener(OnDictationSessionStopped);
    ...
    private void OnDictationSessionStopped(DictationSession session)
    {
        if (!string.IsNullOrWhiteSpace(currentTranscript))
        {
            ...
            chatBoxManager.AddMessage(currentTranscript, true);
            ...
            chatBoxManager.StartCoroutine(chatBoxManager.SendToN8N(payload));
        }
