...
public class GetAppList : MonoBehaviour
{
    ...
    // web view when laucnched
    public GameObject webview;
    public GameObject gmailwebview;
        ...
        if (request.result == UnityWebRequest.Result.Success) {
            ...
            if (response.success)
            {
                ...
                if (launchedApp != null && !string.IsNullOrEmpty(launchedApp.bundleId))
                {
                    ...
                    //AppPanel newPanel = Instantiate(appPanelPrefab, pos, rot);
                    //newPanel.aPM = aPM; // assign your ActivePanelManager
                    //newPanel.Initialize(launchedApp.name, launchedApp.bundleId);
                    //aPM.ShowPanel(newPanel.gameObject);
                    //newPanel.StartScreenShare();
                    if (!isStarted)
                    {
                        aPM.TogglePanel(webview);
                        isStarted = true;
                    }
                    if (launchedApp.bundleId == "com.apple.mail") aPM.TogglePanel(gmailwebview);
                }
                else
                {
